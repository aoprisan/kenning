...
function mkTexts(g, n, size, fill) {
    while (g.firstChild)
        g.removeChild(g.firstChild);
    const out = [];
    for (let i = 0; i < n; i++) {
...
const A3_INSTR = [
    { txt: "add r1,r2,r3", src: [], load: false },
    { txt: "sub r4,r1,r5", src: [0], load: false }, // RAW on r1
    { txt: "or  r6,r7,r8", src: [], load: false },
    { txt: "lw  r9,(r10)", src: [], load: true },
...
const A4_N = 24;
function a4Outcome(pattern, i) {
    if (pattern === "loop")
        return i % 8 !== 7; // seven taken, one not
    if (pattern === "alt")
        return i % 2 === 0;
...
export const anims = {
  ...
  <text data-e="cost" x="170" y="188" font-size="8.5" fill="${C_L3}" text-anchor="middle" font-family="IBM Plex Mono"></text>
        ...
        draw(t, c, E) {
            ...
            for (let i = 0; i < A4_N; i++) {
                ...
                A.setAttribute("x", x.toFixed(1));
                A.setAttribute("y", "22");
                P.setAttribute("x", x.toFixed(1));
                P.setAttribute("y", "46");
                M.setAttribute("x", x.toFixed(1));
                M.setAttribute("y", "70");
  ...
  <text data-e="cnt" x="170" y="210" font-size="8.5" fill="${C_L3}" text-anchor="middle" font-family="IBM Plex Mono"></text>
        ...
        draw(t, c, E) {
            ...
            for (let i = 0; i < upto; i++) {
                ...
                if (at >= 0) {
                    hits++;
                    contents[set].splice(at, 1);
                    contents[set].push(tag);
                }
                else {
                    misses++;
                    contents[set].push(tag);
                    if (contents[set].length > W)
                        contents[set].shift();
                }
            }
            for (let s = 0; s < A6_SETS; s++) {
                const lab = E._idx[s];
                lab.setAttribute("x", "22");
                lab.setAttribute("y", String(30 + s * 8.6));
  ...
  <text data-e="cost" x="170" y="196" font-size="9" fill="${C_L2}" text-anchor="middle" font-family="IBM Plex Mono"></text>
        ...
        draw(t, c, E) {
            ...
            const order = (i) => pat === "seq" ? i // 0…7: one line, every byte used
            ...
            for (let l = 0; l < A7_LINES; l++) {
                ...
                const m = E._marks[l];
                m.setAttribute("x", "68");
                m.setAttribute("y", String(36 + l * 10));
                m.setAttribute("width", "66");
                m.setAttribute("height", "2");
  ...
  <text data-e="sub" x="170" y="204" font-size="8.5" fill="${C_L3}" text-anchor="middle" font-family="IBM Plex Mono"></text>
        ...
        draw(t, c, E) {
            ...
            const X = (l) => 44 + 278 * (Math.min(7, Math.max(-5, l)) + 5) / 12;
