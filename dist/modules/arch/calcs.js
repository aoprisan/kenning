...
const n = (v, d = 0) => isFinite(v)
    ...
    : "—";
const isPow2 = (v) => v >= 1 && Number.isInteger(v) && (v & (v - 1)) === 0;
...
export const calcs = {
    ...
    float: {
        ...
        run({ v, fmt }) {
            ...
            if (kind === "normal")
                lines.push(["Unbiased exponent", String(exp - BIAS)]);
            else if (kind === "subnormal")
                lines.push(["Unbiased exponent", String(1 - BIAS) + " (subnormal: no implicit 1)"]);
    ...
    pipe: {
        ...
        run({ stages, instr, stall, base, oh }) {
            if (!(stages >= 1) || !(instr >= 1) || !(base > 0))
                return { lines: [["Fill in the fields", "—"]] };
            if (stall < 0 || oh < 0)
                return { lines: [["Stalls and overhead cannot be negative", "—"]] };
            if (stages > 100)
                return { lines: [["Keep the stage count under 100", "—"]],
    ...
    branch: {
        ...
        run({ acc, penalty, freq, ipc }) {
            if (acc < 0 || acc > 100)
                return { lines: [["Accuracy must be between 0 and 100", "—"]] };
            if (!(penalty >= 0) || !(freq >= 0) || !(ipc > 0))
                return { lines: [["Fill in the fields", "—"]] };
            if (freq > 100)
                return { lines: [["At most 100 branches per 100 instructions", "—"]] };
    ...
    cache: {
        ...
        run({ size, line, ways, abits, stride }) {
            if (!isPow2(line))
                return { lines: [["Line size must be a power of two", "—"]],
            ...
            for (let i = 0; i < probes; i++)
                seen.add(Math.floor((i * stride) / line) % sets);
    ...
    roofline: {
        ...
        run({ peak, bw, ai, got }) {
            if (!(peak > 0) || !(bw > 0))
                return { lines: [["Peak compute and bandwidth must be positive", "—"]] };
            if (!(ai > 0))
                return { lines: [["Arithmetic intensity must be positive", "—"]],
            ...
            if (got > 0) {
                lines.push(["Measured", got.toPrecision(4) + " GFLOP/s"], ["Fraction of the roofline achieved", (100 * got / attainable).toFixed(1) + " %"]);
                if (got < attainable * 0.5)
                    return { lines,
