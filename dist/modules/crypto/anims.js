...
const word = (...xs) => mix(xs.reduce((a, v) => mix((a ^ (v + 0x165667b1)) >>> 0), 0x5bf03635));
/** The low byte of it, which is what the pictures colour by. */
const byteOf = (...xs) => word(...xs) & 255;
...
const k3x = (i) => 62 + i * (332 - 62) / (K3_BITS - 1);
/* ---------- k4 · the plaintext image, as a padlock ---------- */
const K4_W = 20, K4_H = 12;
const K4_PLAIN = (() => {
    const out = [];
    for (let r = 0; r < K4_H; r++)
        for (let c = 0; c < K4_W; c++) {
...
export const anims = {
  ...
  <text data-e="xmax" x="326" y="200" font-size="8" fill="${C_L3}" text-anchor="end" font-family="IBM Plex Mono"></text>
        ...
        draw(t, c, E) {
            ...
            const X = (k) => 48 + 278 * Math.min(1, Math.max(0, k / bits));
  ...
  <text data-e="sub" x="170" y="186" font-size="8.5" fill="${C_L3}" text-anchor="middle" font-family="IBM Plex Mono"></text>
        ...
        draw(t, c, E) {
            ...
            const k1 = (i) => byteOf(7, i) & 1;
            const k2 = (i) => same ? k1(i) : byteOf(19, i) & 1;
