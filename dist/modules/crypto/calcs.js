...
export const calcs = {
    ...
    work: {
        ...
        run({ bits, rate, mach, watt }) {
            if (!(bits > 0) || !(rate > 0) || !(mach > 0))
                return { lines: [["Fill in the fields", "—"]] };
            if (bits > 4096)
                return { lines: [["Security level", "above 4096 bits is not a meaningful input"]],
            ...
            if (lYears < -0.5)
                return { lines, bad: "Broken. This is not a security level, it is a delay." };
            if (lYears < 2)
                return { lines, bad: "Within reach of a determined attacker with rented hardware." };
            if (lYears < 6)
                return { lines, ok: "Expensive today. Not a margin to design a long-lived system around." };
    ...
    birthday: {
        ...
        run({ bits, n: cnt }) {
            if (!(bits > 0) || !(cnt > 0))
                return { lines: [["Fill in the fields", "—"]] };
            if (bits > 4096)
                return { lines: [["Value size", "above 4096 bits is not a meaningful input"]],
            ...
            if (p > 0.5)
                return { lines, bad: "A collision is more likely than not. For a nonce that is a keystream reuse; for a hash it is a forged pair." };
            if (p > 1e-6)
                return { lines, bad: "Too likely to ignore. Use a counter instead of a random value, or widen it." };
    ...
    hash: {
        ...
        run({ alg, rate }) {
            ...
            if (md)
                return { lines, bad: "MD5's collision resistance is gone entirely — chosen-prefix collisions run in minutes, far below the generic figure above. Its preimage resistance is not broken, which is why leaked MD5 password hashes are still attacked by guessing." };
            if (broken)
                return { lines, bad: "SHA-1 collisions are practical: SHAttered found one at about 2⁶³ in 2017, and chosen-prefix collisions followed in 2020. The generic figure above no longer applies. Preimage resistance is intact." };
            if (bits < 256)
                return { lines, ok: "No practical break, but the collision margin is thinner than the modern floor. Prefer 256 bits or more for anything an attacker gets to choose the input of." };
    ...
    rsa: {
        ...
        run({ p, q, e, m }) {
            ...
            if (!isPrime(p) || !isPrime(q))
                return { lines: [["p and q must both be prime", "—"]],
                    bad: "Try 61 and 53, or any two primes below 5000." };
            if (p === q)
                return { lines: [["p and q must be different", "—"]],
                    bad: "With p = q the modulus is a square and φ(n) is not (p−1)(q−1)." };
            if (p > 5000 || q > 5000)
                return { lines: [["Keep p and q below 5000", "—"]],
                    bad: "Above that the modular arithmetic loses exactness in a float." };
            const N = p * q, phi = (p - 1) * (q - 1);
            if (!Number.isInteger(e) || e < 3 || e >= phi)
                return { lines: [["Public exponent", "must satisfy 3 ≤ e < φ(n) = " + n(phi)]],
                    bad: "Real keys use 65537; here it has to fit inside φ(n)." };
            if (gcd(e, phi) !== 1)
                return { lines: [["e and φ(n) share a factor of " + gcd(e, phi), "—"]],
    ...
    pw: {
        ...
        run({ set, len, rate, cost }) {
            ...
            if (!(len > 0) || !(rate > 0) || !(alpha > 1))
                return { lines: [["Fill in the fields", "—"]] };
            if (len > 512)
                return { lines: [["Length", "keep it under 512"]], bad: "Beyond that the answer is the same: astronomically large." };
            ...
            if (bits < 40)
                return { lines, bad: "Falls to a wordlist regardless of the hash. A work factor buys time here, not safety — the whole space is small enough to enumerate." };
            if (lSlow - YEAR < 1)
                return { lines, bad: "Under a year of targeted cracking. Raise the length, not the character classes: length is worth more per keystroke." };
