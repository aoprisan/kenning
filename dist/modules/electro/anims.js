...
import { mkDots, along, wave, axis, C_L1, C_L2, C_L3, C_N, C_PE, C_CU, C_RED, C_GREY, } from "../../anim/runtime.js";
export const anims = {
    /* ---------- m1 · circuitul ---------- */
    m1: {
  ...
  <text x="170" y="36" font-size="11" fill="#5A605A" text-anchor="middle" font-family="IBM Plex Mono">sensul curentului →</text>
        ...
        init(E) { E._d = mkDots(E.dots, 26, 3.2, C_CU); },
        draw(t, c, E) {
  ...
  <text x="190" y="192" font-size="10" fill="${C_CU}" font-family="IBM Plex Mono">curent</text>
        ...
        draw(t, c, E) {
            ...
            E.vu.setAttribute("x2", 62 + 46 * Math.cos(w));
            E.vu.setAttribute("y2", 100 - 46 * Math.sin(w));
            E.pu.setAttribute("cx", 62 + 46 * Math.cos(w));
            E.pu.setAttribute("cy", 100 - 46 * Math.sin(w));
            E.vi.setAttribute("x2", 62 + 34 * Math.cos(w + ph));
            E.vi.setAttribute("y2", 100 - 34 * Math.sin(w + ph));
            E.pi.setAttribute("cx", 62 + 34 * Math.cos(w + ph));
            E.pi.setAttribute("cy", 100 - 34 * Math.sin(w + ph));
  ...
  <text data-e="tc" font-size="9.5" fill="${C_PE}" font-family="IBM Plex Mono" opacity="0">Q compensat</text>
        ...
        draw(t, c, E) {
            ...
            E.lp.setAttribute("x1", x0);
            E.lp.setAttribute("y1", y0);
            E.lp.setAttribute("x2", px);
            E.lp.setAttribute("y2", y0);
            E.lq.setAttribute("x1", px);
            E.lq.setAttribute("y1", y0);
            E.lq.setAttribute("x2", px);
            E.lq.setAttribute("y2", qy);
            E.ls.setAttribute("x1", x0);
            E.ls.setAttribute("y1", y0);
            E.ls.setAttribute("x2", px);
            E.ls.setAttribute("y2", qy);
            E.lc.setAttribute("x1", px + 9);
            E.lc.setAttribute("y1", qy);
            E.lc.setAttribute("x2", px + 9);
            E.lc.setAttribute("y2", q2y);
            E.lc.setAttribute("opacity", Q > Q2 ? puls : 0);
            E.tc.setAttribute("opacity", Q > Q2 ? puls : 0);
            E.tc.setAttribute("x", px + 14);
            E.tc.setAttribute("y", (qy + q2y) / 2);
            E.tp.setAttribute("x", (x0 + px) / 2);
            E.tp.setAttribute("y", y0 + 14);
            E.tq.setAttribute("x", px + 4);
            E.tq.setAttribute("y", (y0 + qy) / 2);
            E.ts.setAttribute("x", (x0 + px) / 2 - 14);
            E.ts.setAttribute("y", (y0 + qy) / 2 - 4);
  ...
  <text x="226" y="199" font-size="9.5" fill="${C_N}" font-family="IBM Plex Mono">neutru</text>
        ...
        draw(t, c, E) {
            ...
            const ang = (k) => w - k * 2 * Math.PI / 3;
            let nx = 0, ny = 0;
            [0, 1, 2].forEach(k => {
                const r = 52 * A[k], x = 76 + r * Math.cos(ang(k)), y = 105 - r * Math.sin(ang(k));
                const L = E["v" + (k + 1)];
                L.setAttribute("x1", 76);
                L.setAttribute("y1", 105);
                L.setAttribute("x2", x);
                L.setAttribute("y2", y);
                nx += r * Math.cos(ang(k));
                ny += r * Math.sin(ang(k));
            ...
            E.vn.setAttribute("x1", 76);
            E.vn.setAttribute("y1", 105);
            E.vn.setAttribute("x2", 76 - nx);
            E.vn.setAttribute("y2", 105 + ny);
            E.pn.setAttribute("cx", 76 - nx);
            E.pn.setAttribute("cy", 105 + ny);
            wave(E.wn, x2 => {
                const p = (x2 - 158) * 0.036;
                let s = 0;
                [0, 1, 2].forEach(k => s += A[k] * Math.sin(w - p - k * 2 * Math.PI / 3));
                return 105 + 46 * s;
            }, 158, 332, 3);
  ...
  <text data-e="lbl" x="183" y="182" font-size="10" fill="${C_L2}" font-family="IBM Plex Mono" text-anchor="middle"></text>
        ...
        init(E) { E._acc = 0; E._last = 0; },
        draw(t, c, E) {
            const k = +c.curba;
            const X = (m) => 40 + 286 * Math.log10(m) / Math.log10(30);
            const Y = (s) => 166 - 148 * Math.log10(s / 0.01) / Math.log10(3600 / 0.01);
            const trip = (m) => m < 1.13 ? Infinity : (m >= k ? 0.01 : Math.max(0.02, 3600 * Math.pow(m / 1.45, -9.7)));
            let d = `M${X(1.13)} ${Y(3600)}`;
            for (let m = 1.2; m < k; m *= 1.06)
                d += `L${X(m)} ${Y(Math.min(3600, trip(m)))}`;
            d += `L${X(k)} ${Y(Math.min(3600, trip(k * 0.999)))}L${X(k)} ${Y(0.01)}L${X(30)} ${Y(0.01)}`;
            E.cv.setAttribute("d", d);
            const T = trip(c.m);
            E.vl.setAttribute("x1", X(c.m));
            E.vl.setAttribute("x2", X(c.m));
            E.pt.setAttribute("cx", X(c.m));
            E.pt.setAttribute("cy", Y(Math.min(3600, isFinite(T) ? T : 3600)));
            E.pt.setAttribute("opacity", isFinite(T) ? 1 : 0.25);
            const dt = Math.min(0.05, t - E._last);
            E._last = t;
            if (isFinite(T)) {
                E._acc += dt;
                if (E._acc > Math.min(T, 6) + 0.9)
                    E._acc = 0;
            }
            else
                E._acc = 0;
  ...
  <text data-e="trip" x="199" y="42" font-size="12" fill="${C_RED}" text-anchor="middle" font-family="IBM Plex Mono" opacity="0">DECLANȘAT</text>
        ...
        init(E) { E._f = mkDots(E.df, 16, 3, C_L1); E._n = mkDots(E.dn, 16, 3, C_N); E._g = mkDots(E.dg, 7, 3, C_RED); },
        draw(t, c, E) {
  ...
  <text data-e="sep" x="160" y="166" font-size="8.5" fill="${C_PE}" text-anchor="middle" font-family="IBM Plex Mono" opacity="0">separare PEN</text>
        ...
        init(E) { E._d = mkDots(E.fd, 9, 3.4, C_RED); },
        draw(t, c, E) {
            const S = c.s;
  ...
  <text data-e="tt" font-size="10" fill="${C_L2}" font-family="IBM Plex Mono" text-anchor="middle"></text>
        ...
        draw(t, c, E) {
            ...
            const V = (T) => c.ns * c.voc * (1 + beta / 100 * (T - 25));
            const lo = Math.min(V(60), VM) * 0.86, hi = Math.max(V(-25), VM) * 1.04;
            const X = (T) => 42 + 284 * (T + 25) / 85, Y = (v) => 168 - 152 * (v - lo) / (hi - lo);
            let d = "";
            for (let T = -25; T <= 60; T += 2.5)
                d += (d ? "L" : "M") + X(T) + " " + Y(V(T));
            E.cv.setAttribute("d", d);
            E.lim.setAttribute("y1", Y(VM));
            E.lim.setAttribute("y2", Y(VM));
            E.limT.setAttribute("y", Y(VM) - 5);
            const T = 17.5 - 42.5 * Math.cos(t * 0.55), v = V(T);
            E.pt.setAttribute("cx", X(T));
            E.pt.setAttribute("cy", Y(v));
            E.pt.setAttribute("fill", v > VM ? C_RED : C_L2);
            E.tt.setAttribute("x", Math.min(300, Math.max(60, X(T))));
            E.tt.setAttribute("y", Y(v) - 13);
  ...
  <text x="188" y="186" font-size="9.5" fill="${C_CU}" font-family="IBM Plex Mono">curentul real</text>
        ...
        draw(t, c, E) {
            ...
            wave(E.sum, x => {
                const p = w - (x - 14) * k;
                return 98 - 52 * (Math.sin(p) + a3 * Math.sin(3 * p) + a5 * Math.sin(5 * p)) / (1 + a3 * 0.5 + a5 * 0.4);
            }, 14, 328, 2);
