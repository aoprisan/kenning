...
function ns(v) {
    ...
    if (v < 1e6)
        return (v / 1e3).toPrecision(3) + " µs";
    if (v < 1e9)
        return (v / 1e6).toPrecision(3) + " ms";
    if (v < 6e10)
        return (v / 1e9).toPrecision(3) + " s";
    if (v < 3.6e12)
        return (v / 6e10).toPrecision(3) + " minutes";
    if (v < 8.64e13)
        return (v / 3.6e12).toPrecision(3) + " hours";
    if (v < 3.15e16)
        return (v / 8.64e13).toPrecision(3) + " days";
...
export const calcs = {
    ...
    sched: {
        ...
        run({ a, b, c, d, q }) {
            const jobs = [["A", a], ["B", b], ["C", c], ["D", d]];
            if (jobs.some(([, v]) => !(v > 0)))
                return { lines: [["Every job needs a positive length", "—"]] };
            if (!(q > 0))
                return { lines: [["The quantum must be positive", "—"]] };
            if (jobs.some(([, v]) => v > 1e6) || q > 1e6)
                return { lines: [["Keep the numbers under a million ms", "—"]],
            ...
            const first = jobs.map(() => null);
            const done = jobs.map(() => 0);
            ...
            const rr = {
                ...
                resp: first.reduce((s, v) => s + (v ?? 0), 0) / jobs.length,
            };
    ...
    xlate: {
        ...
        run({ va, psize, vabits }) {
            ...
            if (!(va >= 0) || !Number.isFinite(va))
                return { lines: [["Enter a non-negative address", "—"]] };
            const levels = Math.round((width - offBits) / IDX);
            if (levels < 1)
                return { lines: [["That page size leaves no levels to walk at this address width", "—"]],
                    bad: "A 1 GiB page in a 48-bit space walks two levels; there is no configuration with fewer." };
            const max = 2 ** width;
            if (va >= max)
                return { lines: [["Address is outside a " + width + "-bit space", "—"]],
            ...
            for (const [name, v] of idx)
                lines.push(["  " + name + " index", String(v)]);
            lines.push(["Offset within page", n(offset)], ["Virtual page number", n(vpn)], ["Memory accesses on a TLB miss", String(levels) + " for the walk, then 1 for the data"], ["TLB reach at " + TLB + " entries", bytes(TLB * pageSize)], ["Page tables to map 1 GiB", n(tables) + " tables = " + bytes(tables * 4096)]);
            if (offBits === 12)
                return { lines,
    ...
    amat: {
        ...
        run({ hit, levels, mem, fault, svc }) {
            ...
            if (hit < 0 || hit > 100)
                return { lines: [["TLB hit rate must be between 0 and 100", "—"]],
                    bad: "A rate outside that range is not a rate." };
            if (fault < 0 || svc < 0)
                return { lines: [["Fault rate and service time cannot be negative", "—"]] };
            const pHit = hit / 100;
    ...
    amdahl: {
        ...
        run({ p, N, alpha, beta }) {
            if (!(N >= 1))
                return { lines: [["Need at least one processor", "—"]] };
            if (!(p >= 0) || p > 1)
                return { lines: [["Parallel fraction must be between 0 and 1", "—"]],
                    bad: "It is a fraction of the work, not a percentage." };
            if (alpha < 0 || beta < 0)
                return { lines: [["α and β cannot be negative", "—"]] };
            if (N > 1e6)
                return { lines: [["Keep the processor count under a million", "—"]] };
            const amdahl = 1 / ((1 - p) + p / N);
            const ceiling = p >= 1 ? Infinity : 1 / (1 - p);
            const usl = (k) => k / (1 + alpha * (k - 1) + beta * k * (k - 1));
            const c = usl(N);
            ...
            if (isFinite(peakN)) {
                lines.push(["USL peaks at", n(Math.round(peakN)) + " processors"], ["Peak speedup", usl(peakN).toFixed(2) + "×"]);
            }
            else {
                lines.push(["USL peaks at", "no turnover — β is zero"]);
            }
            if (isFinite(peakN) && N > peakN)
                return { lines,
    ...
    latency: {
        ...
        run({ op, count }) {
            ...
            if (!(count > 0))
                return { lines: [["Enter a positive count", "—"]] };
            if (count > 1e15)
                return { lines: [["Keep the count under 10¹⁵", "—"]] };
            const L1 = 1;
