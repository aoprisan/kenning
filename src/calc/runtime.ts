...
export function renderCalc(c: Calc | undefined): string {
  if (!c) return `<p class="empty">Modulul acesta nu are calculator — teoria lui e conceptuală.</p>`;
...
export function wireCalc(c: Calc | undefined, root: HTMLElement): void {
  if (!c) return;
  ...
  const read = (): void => {
    ...
    for (const fl of c.fields) {
      const el = root.querySelector<HTMLInputElement>("#f_" + fl.k);
      if (!el) continue;
