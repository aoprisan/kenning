...
d4:{
 ...
 draw(t: number, c: { N: number; W: number; R: number }, E: Els){
  ...
  const inRead = (i: number): boolean => {
    for (let k = 0; k < R; k++) if ((off + k) % N === i) return true;
    return false;
  };
  let hit = 0;
  for (let i = 0; i < W; i++) if (inRead(i)) hit++;
  const fresh = hit > 0;
