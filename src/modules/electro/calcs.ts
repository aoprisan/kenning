...
ohm:{
  ...
  run(f){
    let {U,I,R}=f; const out: [string,string][] = [];
    if(U&&I){R=U/I;} else if(U&&R){I=U/R;} else if(I&&R){U=I*R;}
    else return {lines:[["Introdu două valori","—"]]};
    const P=U*I;
    out.push(["Tensiune U",num(U,1)+" V"],["Curent I",num(I,2)+" A"],
             ["Rezistență R",num(R,2)+" Ω"],["Putere P",num(P,1)+" W"],
             ["Energie în 24 h",num(P*24/1000,2)+" kWh"]);
    return {lines:out};
  }},
...
cosfi:{
  ...
  run({P,c1,c2,U}){
    if(!P||!c1||!c2) return {lines:[["Completează câmpurile","—"]]};
    if(c1>1||c2>1||c1<=0||c2<=0) return {lines:[["cos φ trebuie să fie între 0 și 1","—"]],bad:"Valoare imposibilă pentru factorul de putere."};
...
trifazat:{
  ...
  run({U,I,c}){
    if(!U||!I||!c) return {lines:[["Completează câmpurile","—"]]};
...
cadere:{
  ...
  run({mode,mat,L,I,S,c,lim}){
    if(!L||!I||!S) return {lines:[["Completează câmpurile","—"]]};
...
scc:{
  ...
  run({U,In,curba,Zs}){
    if(!U||!In||!Zs) return {lines:[["Completează câmpurile","—"]]};
...
tt:{
  ...
  run({Ra,Idn}){
    if(!Ra||!Idn) return {lines:[["Completează câmpurile","—"]]};
...
pv:{
  ...
  run({Ns,Voc,beta,Tmin,Vmax}){
    if(!Ns||!Voc) return {lines:[["Completează câmpurile","—"]]};
    const Vstc = Ns*Voc;
...
ev:{
  ...
  run({P,faze,L}){
    if(!P) return {lines:[["Completează câmpurile","—"]]};
    const tri = faze==="3";
    const U = tri?400:230;
