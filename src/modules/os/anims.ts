...
/** One (job, start, end) run of the CPU. */
type Seg = { j: number; s: number; e: number };
...
o3:{
 ...
 draw(t: number, c: { policy: string; q: number }, E: Els){
  ...
  if (pol === "rr") {
    ...
    if (pol === "sjf") order.sort((x, y) => O3_JOBS[x].len - O3_JOBS[y].len);
    let now = 0;
    for (const i of order) { segs.push({ j: i, s: now, e: now + O3_JOBS[i].len }); now += O3_JOBS[i].len; }
  }

  // All jobs arrive at 0, so turnaround is completion and response is first run.
  const firstRun: number[] = [NaN, NaN, NaN, NaN];
  const done: number[] = [0, 0, 0, 0];
  for (const s of segs) {
    if (!isFinite(firstRun[s.j])) firstRun[s.j] = s.s;
    done[s.j] = s.e;
  }
  const avgTurn = done.reduce((a, b) => a + b, 0) / 4;
  const avgResp = firstRun.reduce((a, b) => a + (isFinite(b) ? b : 0), 0) / 4;

  const X = (u: number): number => 40 + 284 * u / O3_TOTAL;
  const nowU = (t % 7) / 7 * O3_TOTAL;

  for (let i = 0; i < O3_SEGS; i++) {
    const el = E._bars[i], s = segs[i];
    if (!s) { el.setAttribute("opacity", "0"); continue; }
...
o4:{
 ...
 draw(t: number, c: { tlb: string; psize: string }, E: Els){
  const hit = c.tlb === "hit";
  ...
  const idx: number[] = [];
  for (let i = levels - 1; i >= 0; i--) idx.push(Math.floor(vpn / 512 ** i) % 512);
  ...
  const stops: [number, number][] = hit
    ? [[54, 78], [307, 123]]
    : [[54, 78], ...idx.map((_, i): [number, number] => [48 + i * 68, 123]), [307, 123]];
...
o5:{
 ...
 draw(t: number, c: { policy: string; frames: number }, E: Els){
  ...
  const mem: number[] = [];
  const loadedAt: number[] = [];   // FIFO age
  const usedAt: number[] = [];     // LRU recency
  const faultAt: boolean[] = [];
  let faults = 0;

  for (let i = 0; i < O5_REF.length; i++) {
    const page = O5_REF[i];
    const at = mem.indexOf(page);
    if (at >= 0) { usedAt[at] = i; faultAt[i] = false; continue; }
    faults++; faultAt[i] = true;
    if (mem.length < F) { mem.push(page); loadedAt.push(i); usedAt.push(i); continue; }
    let victim = 0;
    if (pol === "fifo") {
      for (let k = 1; k < mem.length; k++) if (loadedAt[k] < loadedAt[victim]) victim = k;
    } else if (pol === "lru") {
      for (let k = 1; k < mem.length; k++) if (usedAt[k] < usedAt[victim]) victim = k;
    } else {
      // OPT: evict whichever resident page is next needed furthest ahead.
      let best = -1;
      for (let k = 0; k < mem.length; k++) {
        let next = Infinity;
        for (let j = i + 1; j < O5_REF.length; j++) if (O5_REF[j] === mem[k]) { next = j; break; }
        if (next > best) { best = next; victim = k; }
      }
    }
    mem[victim] = page; loadedAt[victim] = i; usedAt[victim] = i;
    if (i >= steps) break;
  }

  // Contents as of `steps`, replayed again so the display matches the cursor.
  const shown: number[] = [];
  const sLoaded: number[] = [], sUsed: number[] = [];
  let shownFaults = 0;
  for (let i = 0; i < Math.min(steps, O5_REF.length); i++) {
    const page = O5_REF[i], at = shown.indexOf(page);
    if (at >= 0) { sUsed[at] = i; continue; }
    shownFaults++;
    if (shown.length < F) { shown.push(page); sLoaded.push(i); sUsed.push(i); continue; }
    let victim = 0;
    if (pol === "fifo") { for (let k = 1; k < shown.length; k++) if (sLoaded[k] < sLoaded[victim]) victim = k; }
    else if (pol === "lru") { for (let k = 1; k < shown.length; k++) if (sUsed[k] < sUsed[victim]) victim = k; }
    else {
      let best = -1;
      for (let k = 0; k < shown.length; k++) {
        let next = Infinity;
        for (let j = i + 1; j < O5_REF.length; j++) if (O5_REF[j] === shown[k]) { next = j; break; }
        if (next > best) { best = next; victim = k; }
      }
    }
    shown[victim] = page; sLoaded[victim] = i; sUsed[victim] = i;
  }

  for (let i = 0; i < O5_REF.length; i++) {
    const x = 44 + i * 23, past = i < steps;
    const r = E._refs[i];
    r.setAttribute("x", String(x)); r.setAttribute("y", "12");
    r.setAttribute("fill", past ? (faultAt[i] ? C_RED : C_PE) : C_GREY);
    r.setAttribute("fill-opacity", past ? "0.9" : "0.3");
    const m = E._marks[i];
    m.setAttribute("x", String(x)); m.setAttribute("y", "36");
    m.setAttribute("opacity", past && faultAt[i] ? "1" : "0");
  }

  for (let k = 0; k < 5; k++) {
    const s = E._slots[k];
    s.setAttribute("x", "44"); s.setAttribute("y", String(56 + k * 24));
...
o7:{
 ...
 draw(t: number, c: { shift: number; lock: string }, E: Els){
  ...
  for (let s = 0; s < SLOTS; s++) {
    ...
    if (s === bAt[0]) regB = counter;
    if (s === bAt[1]) regB = regB + 1;
    if (s === bAt[2]) counter = regB;
    if (s === aAt[0]) regA = counter;
    if (s === aAt[1]) regA = regA + 1;
    if (s === aAt[2]) counter = regA;
    trace[s] = counter;
  }
  const lost = counter < 2;

  const X = (s: number): number => 26 + s * 37;
...
o11:{
 ...
 draw(t: number, c: { mode: string; crash: number }, E: Els){
  const mode = String(c.mode);
  const crash = Math.min(1, Math.max(0, c.crash / 100));
  const X = (u: number): number => 26 + 300 * u;

  const durable = O11_ISSUE.map((iss) =>
    mode === "each" ? Math.min(0.97, iss + 0.05)
      : mode === "batch" ? 0.66
      : 0.88);
