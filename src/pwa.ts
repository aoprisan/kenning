...
/** The half of `BeforeInstallPromptEvent` we use. Not in lib.dom yet. */
...
function toast(message: string, action: { label: string; run: () => void } | null): void {
...
export function initPWA(): void {
  ...
  navigator.serviceWorker.register("./sw.js", { scope: "./", updateViaCache: "none" })
    .then(watchForUpdate)
    .catch(() => { /* insecure origin, file://, or storage denied — run online only */ });
}
