...
export interface QuizState {
  /** Module id, or "__exam__" for the mixed exam. */
  mod: string;
  qs: Question[];
  i: number;
  correct: number;
...
export function startQuiz(all: readonly Question[], mod: string): QuizState {
...
export function renderQuiz(q: QuizState): string {
  const { qs, i, correct, picked } = q;
  if (!qs.length) return `<p class="empty">Nu există încă întrebări pentru acest modul.</p>`;
